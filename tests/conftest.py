...
def client(mock_orchestrator, mock_registry, mock_scheduler):
    ...
    test_app = FastAPI(
        title="Test Cloud Artifact Extractor",
        description="Test API",
        version="1.0.0"
    )
    
    # Include the same routers
    test_app.include_router(extraction.router, prefix="/api/v1/extraction", tags=["extraction"])
    test_app.include_router(schedules.router, prefix="/api/v1/schedules", tags=["schedules"])
...
def mock_orchestrator():
    ...
    mock.get_job_status = Mock(return_value=Job(
        id="test-job",
