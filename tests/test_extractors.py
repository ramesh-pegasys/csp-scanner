...
def test_ec2_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts EC2 instances, security groups, and network interfaces"
    assert metadata.resource_types == ["instance", "security-group", "network-interface"]
    assert metadata.supports_regions is True
...
async def test_ec2_extractor_extract_single_region():
    ...
    with patch.object(extractor, '_extract_region', return_value=mock_instances) as mock_extract:
...
async def test_ec2_extractor_extract_multiple_regions():
    ...
    with patch.object(extractor, '_get_all_regions', return_value=['us-east-1', 'us-west-2']), \
...
def test_apigateway_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts API Gateway REST APIs, resources, methods, and deployments"
    assert metadata.resource_types == ["rest-api", "resource", "method", "deployment", "stage"]
    assert metadata.supports_regions is True
    assert metadata.requires_pagination is True
...
async def test_apigateway_extractor_extract_with_region():
    ...
    with patch.object(extractor, '_extract_rest_apis', return_value=mock_apis) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
async def test_apprunner_extractor_extract():
    ...
    with patch.object(extractor, '_extract_services', return_value=mock_services) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
def test_cloudfront_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts CloudFront distributions, origins, and cache behaviors"
...
async def test_cloudfront_extractor_extract():
    ...
    mock_distributions = [{'resource_id': 'dist1', 'resource_type': 'cloudfront:distribution'}]
    with patch.object(extractor, '_extract_distributions', return_value=mock_distributions) as mock_extract:
        artifacts = await extractor.extract()
...
def test_ecs_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts ECS clusters, services, tasks, and task definitions"
...
async def test_ecs_extractor_extract():
    ...
    with patch.object(extractor, '_extract_clusters', return_value=mock_clusters) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
def test_eks_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts EKS clusters, node groups, and Fargate profiles"
...
async def test_elb_extractor_extract():
    ...
    with patch.object(extractor, '_extract_load_balancers', return_value=mock_lbs) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
async def test_iam_extractor_extract():
    ...
    with patch.object(extractor, '_extract_users', return_value=mock_users) as mock_extract:
        artifacts = await extractor.extract()
...
async def test_kms_extractor_extract():
    ...
    with patch.object(extractor, '_extract_keys', return_value=mock_keys) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
def test_lambda_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts Lambda functions, layers, and event source mappings"
...
async def test_lambda_extractor_extract():
    ...
    with patch.object(extractor, '_extract_region', return_value=mock_functions) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
...
def test_rds_extractor_metadata():
    ...
    assert metadata.resource_types == ["db-instance", "db-cluster", "db-snapshot", "db-cluster-snapshot"]
    assert metadata.supports_regions is True
    assert metadata.requires_pagination is True
...
def test_vpc_extractor_metadata():
    ...
    assert metadata.version == "1.0.0"
    assert metadata.description == "Extracts VPCs, subnets, internet gateways, NAT gateways, route tables, and network ACLs"
    assert metadata.resource_types == ["vpc", "subnet", "internet-gateway", "nat-gateway", "route-table", "network-acl"]
    assert metadata.supports_regions is True
    assert metadata.requires_pagination is True
...
async def test_vpc_extractor_extract():
    ...
    with patch.object(extractor, '_extract_region', return_value=mock_vpcs) as mock_extract:
        artifacts = await extractor.extract(region='us-east-1')
