...
from app.transport.retry_policy import (
    RetryConfig, RetryStrategy, JitterType, RetryPolicy, create_retry_policy,
...
@pytest.mark.parametrize("strategy,expected", [
    (RetryStrategy.EXPONENTIAL, [0.0, 4.0, 8.0, 10.0]),  # 2*2^1=4, 2*2^2=8, 2*2^3=16->10 (capped)
...
@pytest.mark.parametrize("jitter_type", [JitterType.NONE, JitterType.FULL, JitterType.EQUAL, JitterType.DECORRELATED])
...
async def test_execute_with_retry_failure():
    config = RetryConfig(max_attempts=2, base_delay=0.01, max_delay=0.1, retryable_exceptions=[DummyException])
    policy = RetryPolicy(config)
    async def op():
...
async def test_execute_with_retry_custom_condition():
    ...
    config = RetryConfig(max_attempts=3, base_delay=0.01, max_delay=0.1, retryable_exceptions=[DummyException], retry_condition=retry_condition)
    policy = RetryPolicy(config)
    calls = []
    async def op():
        calls.append(1)
...
@pytest.mark.parametrize("config_obj", [DEFAULT_RETRY_CONFIG, AGGRESSIVE_RETRY_CONFIG, CONSERVATIVE_RETRY_CONFIG])
def test_preconfigured_configs(config_obj):
    policy = create_retry_policy(config_obj)
    assert isinstance(policy, RetryPolicy)
    assert policy.config == config_obj

@pytest.mark.parametrize("bad_field,bad_value,err_msg", [
    ...
    ("backoff_multiplier", 1, "backoff_multiplier must be > 1 for exponential strategy"),
])
def test_retry_config_validation(bad_field, bad_value, err_msg):
    kwargs = dict(max_attempts=3, base_delay=1.0, max_delay=10.0, backoff_multiplier=2.0, strategy=RetryStrategy.EXPONENTIAL)
