...
class DummyExtractor(BaseExtractor):
    ...
    def transform(self, raw_data):
        return {"resource_id": "dummy", "resource_type": "dummy", "service": "dummy", "configuration": {}}
...
def registry(tmp_path):
    ...
    settings = Settings()
    with patch.object(ExtractorRegistry, "_register_default_extractors", lambda self: None):
...
def test_register_default_extractors(monkeypatch, registry):
    modules = {
        ...
        'app.extractors.lambda_extractor': SimpleNamespace(LambdaExtractor=make_dummy_extractor("lambda")),
        ...
        'app.extractors.apprunner': SimpleNamespace(AppRunnerExtractor=make_dummy_extractor("apprunner")),
        ...
        'app.extractors.cloudfront': SimpleNamespace(CloudFrontExtractor=make_dummy_extractor("cloudfront")),
        'app.extractors.apigateway': SimpleNamespace(APIGatewayExtractor=make_dummy_extractor("apigateway")),
    ...
    expected_services = ['ec2', 's3', 'rds', 'lambda', 'iam', 'vpc', 'apprunner',
