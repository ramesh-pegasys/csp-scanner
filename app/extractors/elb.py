...
class ELBExtractor(BaseExtractor):
    ...
    async def extract(self, region: Optional[str] = None,
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            tasks = [
                loop.run_in_executor(executor, self._extract_load_balancers, region, filters),
                loop.run_in_executor(executor, self._extract_target_groups, region, filters)
            ]
    ...
    def _extract_load_balancers(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate():
                for lb in page['LoadBalancers']:
                    try:
                        ...
                        listeners = []
                        try:
                            listeners_response = client.describe_listeners(LoadBalancerArn=lb_arn)
                            listeners = listeners_response['Listeners']
                        except Exception as e:
                            logger.warning(f"Failed to get listeners for LB {lb['LoadBalancerName']}: {e}")
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to get tags for LB {lb['LoadBalancerName']}: {e}")
                        ...
                        logger.error(f"Failed to extract load balancer {lb.get('LoadBalancerName')}: {e}")
    ...
    def _extract_target_groups(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                for tg in page['TargetGroups']:
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to get targets for TG {tg['TargetGroupName']}: {e}")
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to get tags for TG {tg['TargetGroupName']}: {e}")
                        ...
                        logger.error(f"Failed to extract target group {tg.get('TargetGroupName')}: {e}")
        ...
        if resource_type == 'load-balancer':
            return {
                ...
                'configuration': {
                    ...
                    'load_balancer_addresses': resource.get('LoadBalancerAddresses', []),
