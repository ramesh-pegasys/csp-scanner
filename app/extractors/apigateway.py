...
class APIGatewayExtractor(BaseExtractor):
    ...
    def _extract_rest_apis(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate():
                for api in page['items']:
                    try:
                        ...
                        resources_artifacts = self._extract_resources(client, api_id, region)
                        artifacts.extend(resources_artifacts)

                        # Extract deployments and stages for this API
                        deployments_artifacts = self._extract_deployments(client, api_id, region)
    ...
    def _extract_resources(self, client, api_id: str, region: str) -> List[Dict[str, Any]]:
                ...
                for resource in page['items']:
                    try:
                        ...
                        if resource.get('resourceMethods'):
                            for method_name, method_config in resource['resourceMethods'].items():
                        ...
                        logger.error(f"Failed to extract resource {resource.get('id')} for API {api_id}: {e}")
    ...
    def _extract_deployments(self, client, api_id: str, region: str) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate(restApiId=api_id):
                for deployment in page['items']:
                    try:
                        ...
                        stages_response = client.get_stages(restApiId=api_id, deploymentId=deployment_id)
                        ...
                        logger.error(f"Failed to extract deployment {deployment.get('id')} for API {api_id}: {e}")
        ...
        if resource_type == 'rest-api':
            return {
                ...
                'configuration': {
                    ...
                    'disable_execute_api_endpoint': resource.get('disableExecuteApiEndpoint'),
                },
                'raw': resource
            }
            ...
            return {
                ...
                'configuration': {
                    ...
                    'request_parameters': resource['config'].get('requestParameters', {}),
                    'method_responses': resource['config'].get('methodResponses', {}),
                    'method_integration': resource['config'].get('methodIntegration', {}),
                },
                'raw': resource
            }
