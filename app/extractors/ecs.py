...
class ECSExtractor(BaseExtractor):
    ...
    async def extract(self, region: Optional[str] = None,
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            tasks = [
                ...
                loop.run_in_executor(executor, self._extract_task_definitions, region, filters)
            ]
    ...
    def _extract_clusters(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate():
                for cluster_arn in page['clusterArns']:
                    try:
                        ...
                        logger.error(f"Failed to extract ECS cluster {cluster_arn}: {e}")
    ...
    def _extract_services_in_cluster(self, client, cluster_name: str, region: str) -> List[Dict[str, Any]]:
    ...
    def _extract_tasks_in_cluster(self, client, cluster_name: str, region: str) -> List[Dict[str, Any]]:
    ...
    def _extract_task_definitions(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate():
                for task_def_arn in page['taskDefinitionArns']:
                    try:
                        ...
                        logger.error(f"Failed to extract task definition {task_def_arn}: {e}")
        ...
        if resource_type == 'cluster':
            return {
                ...
                'configuration': {
                    ...
                    'registered_container_instances_count': resource.get('registeredContainerInstancesCount', 0),
            ...
            return {
                ...
                'configuration': {
                    ...
                    'requires_compatibilities': resource.get('requiresCompatibilities', []),
                    'cpu': resource.get('cpu'),
                    'memory': resource.get('memory'),
