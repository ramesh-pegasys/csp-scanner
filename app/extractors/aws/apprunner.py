...
class AppRunnerExtractor(BaseExtractor):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            tasks = [
                ...
                loop.run_in_executor(
                    executor, self._extract_connections, region, filters
                ),
            ]
    ...
    def _extract_services(
        ...
        artifacts = []
        client = self._get_client("apprunner", region=region)
            ...
            while True:
                ...
                for service in response.get("ServiceSummaryList", []):
                    try:
                        ...
                        logger.error(
                            f"Failed to extract App Runner service {service.get('ServiceName')}: {e}"
                        )
            ...
            while True:
                ...
                for connection in response.get("ConnectionSummaryList", []):
                    try:
                        ...
                        details = client.describe_connection(
                        ...
                        logger.error(
                            f"Failed to extract App Runner connection {connection.get('ConnectionName')}: {e}"
                        )
        ...
        if resource_type == "service":
            return {
                ...
                "configuration": {
                    ...
                    "health_check_configuration": resource.get(
                        "HealthCheckConfiguration", {}
                    ),
                    "auto_scaling_configuration": resource.get(
                        "AutoScalingConfigurationSummary", {}
                    ),
                    "network_configuration": resource.get("NetworkConfiguration", {}),
                    "observability_configuration": resource.get(
