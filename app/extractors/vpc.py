...
class VPCExtractor(BaseExtractor):
    ...
    def get_metadata(self) -> ExtractorMetadata:
        return ExtractorMetadata(
            ...
            resource_types=["vpc", "subnet", "internet-gateway", "nat-gateway", "route-table", "network-acl"],
    ...
    def _extract_vpcs(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract VPCs"""
        artifacts = []
    ...
    def _extract_subnets(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract subnets"""
        artifacts = []
    ...
    def _extract_internet_gateways(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract internet gateways"""
        artifacts = []
    ...
    def _extract_nat_gateways(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract NAT gateways"""
        artifacts = []
    ...
    def _extract_route_tables(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract route tables"""
        artifacts = []
    ...
    def _extract_network_acls(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract network ACLs"""
        artifacts = []
        ...
        if resource_type == 'vpc':
            return {
                ...
                'configuration': {
                    ...
                    'cidr_block_association_set': resource.get('CidrBlockAssociationSet', []),
            ...
            return {
                ...
                'configuration': {
                    ...
                    'state': resource.get('State'),
                    'cidr_block': resource.get('CidrBlock'),
                    'ipv6_cidr_block_association_set': resource.get('Ipv6CidrBlockAssociationSet', []),
                    'availability_zone': resource.get('AvailabilityZone'),
                    'availability_zone_id': resource.get('AvailabilityZoneId'),
                    'available_ip_address_count': resource.get('AvailableIpAddressCount'),
                    'default_for_az': resource.get('DefaultForAz'),
                    'map_public_ip_on_launch': resource.get('MapPublicIpOnLaunch'),
                    'assign_ipv6_address_on_creation': resource.get('AssignIpv6AddressOnCreation'),
                    'tags': {tag['Key']: tag['Value'] for tag in resource.get('Tags', [])},
                },
                'raw': resource
            }
