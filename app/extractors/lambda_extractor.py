...
class LambdaExtractor(BaseExtractor):
    ...
    def _extract_region(self, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            event_mappings = self._extract_event_source_mappings(lambda_client, region, filters)
    ...
    def _extract_functions(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_layers(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_event_source_mappings(self, client, region: str, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def transform(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if resource_type == 'function':
            ...
            return {
                ...
                'service': 'lambda',
                'region': region,
                'account_id': resource.get('FunctionArn', '').split(':')[4] if resource.get('FunctionArn') else None,
                'configuration': {
                    ...
                    'maximum_batching_window_in_seconds': resource.get('MaximumBatchingWindowInSeconds'),
                    'parallelization_factor': resource.get('ParallelizationFactor'),
                    'starting_position': resource.get('StartingPosition'),
                    'starting_position_timestamp': resource.get('StartingPositionTimestamp'),
                },
                'raw': resource
            }
