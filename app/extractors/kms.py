            ...
            for page in paginator.paginate():
                for key in page["Keys"]:
                    try:
                        ...
                        policy = {}
                        try:
                            policy_response = client.get_key_policy(
                                KeyId=key_id, PolicyName="default"
                            )
                            policy = policy_response["Policy"]
                        except Exception as e:
                            logger.warning(
                                f"Failed to get policy for key {key_id}: {e}"
                            )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to get rotation status for key {key_id}: {e}"
                            )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to get grants for key {key_id}: {e}"
                            )
                        ...
                        logger.error(
                            f"Failed to extract KMS key {key.get('KeyId')}: {e}"
                        )
            ...
            for page in paginator.paginate():
                for alias in page["Aliases"]:
                    try:
                        ...
                        logger.error(
                            f"Failed to extract KMS alias {alias.get('AliasName')}: {e}"
                        )
