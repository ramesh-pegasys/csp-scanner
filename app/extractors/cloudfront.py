...
class CloudFrontExtractor(BaseExtractor):
    ...
    async def extract(self, region: Optional[str] = None,
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            tasks = [
                ...
                loop.run_in_executor(executor, self._extract_origin_access_identities, filters)
            ]
    ...
    def _extract_distributions(self, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                for distribution_summary in items:
                    try:
                        ...
                        try:
                            ...
                            logger.warning(f"Failed to get tags for distribution {distribution_id}: {e}")
                        ...
                        logger.error(f"Failed to extract CloudFront distribution {distribution_summary.get('Id')}: {e}")
    ...
    def _extract_origin_access_identities(self, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for page in paginator.paginate():
                items = page.get('CloudFrontOriginAccessIdentityList', {}).get('Items', [])
                for oai_summary in items:
                    try:
                        # Get detailed OAI information
                        oai_id = oai_summary['Id']
                        response = client.get_cloud_front_origin_access_identity(Id=oai_id)
                        ...
                        logger.error(f"Failed to extract OAI {oai_summary.get('Id')}: {e}")
