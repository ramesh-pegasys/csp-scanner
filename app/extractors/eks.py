...
class EKSExtractor(BaseExtractor):
            ...
            for page in paginator.paginate():
                for cluster_name in page['clusters']:
                    try:
                        ...
                        logger.error(f"Failed to extract EKS cluster {cluster_name}: {e}")
    ...
    def _extract_nodegroups(self, client, cluster_name: str, region: str) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate(clusterName=cluster_name):
                for nodegroup_name in page['nodegroups']:
                    try:
                        ...
                        logger.error(f"Failed to extract nodegroup {nodegroup_name} in cluster {cluster_name}: {e}")
    ...
    def _extract_fargate_profiles(self, client, cluster_name: str, region: str) -> List[Dict[str, Any]]:
            ...
            for page in paginator.paginate(clusterName=cluster_name):
                for profile_name in page['fargateProfileNames']:
                    try:
                        ...
                        logger.error(f"Failed to extract Fargate profile {profile_name} in cluster {cluster_name}: {e}")

        except Exception as e:
            logger.error(f"Failed to list Fargate profiles for cluster {cluster_name}: {e}")
        ...
        if resource_type == 'cluster':
            return {
                ...
                'configuration': {
                    ...
                    'kubernetes_network_config': resource.get('kubernetesNetworkConfig', {}),
