    ...
    async def extract(
        ...
        with ThreadPoolExecutor(
            ...
            result = await loop.run_in_executor(
                executor, self._extract_storage_accounts, region, filters
            )
            ...
            for account in storage_accounts:
                ...
                if self.config.get("check_blob_encryption", True):
                    try:
                        ...
                        logger.warning(
                            f"Failed to get blob properties for {account.name}: {e}"
                        )
    ...
    def transform(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if resource.encryption:
            ...
            if resource.encryption.services:
                if (
                    hasattr(resource.encryption.services, "blob")
                    and resource.encryption.services.blob
                ):
                    config["encryption"]["services"]["blob"] = {
                        "enabled": resource.encryption.services.blob.enabled
                    }
                if (
                    hasattr(resource.encryption.services, "file")
                    and resource.encryption.services.file
                ):
        ...
        if raw_data.get("blob_properties"):
            ...
            if (
                hasattr(blob_props, "delete_retention_policy")
                and blob_props.delete_retention_policy
            ):
