...
class GCPComputeExtractor(BaseExtractor):
    ...
    def _extract_zone(self, zone: str, gcp_session: GCPSession) -> List[Dict[str, Any]]:
        ...
        if self.config.get("include_instance_groups", True):
            try:
                ...
                logger.error(f"Error extracting managed instance groups from zone {zone}: {str(e)}")
        
        return resources

    def _extract_instances(self, zone: str, gcp_session: GCPSession) -> List[Dict[str, Any]]:
        """
    ...
    def _extract_managed_instance_groups(self, zone: str, gcp_session: GCPSession) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(f"Extracted {len(migs)} managed instance groups from zone {zone}")
            return migs
            
        except Exception as e:
            logger.error(f"Error listing managed instance groups in zone {zone}: {str(e)}")
            return []
        ...
        try:
            ...
            if "instance" in resource_type_suffix:
                ...
                config.update({
                    ...
                    "auto_healing_policies": raw_data.get("auto_healing_policies", []),
                })
