...
class GCPStorageExtractor(BaseExtractor):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                buckets = await loop.run_in_executor(
                    executor, self._extract_buckets, gcp_session, region
                )
    ...
    def _extract_buckets(
        self, gcp_session: GCPSession, location_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for bucket in buckets_list:
                ...
                if (
                    location_filter
                    and bucket.location.lower() != location_filter.lower()
                ):
                    continue
                ...
                if bucket.lifecycle_rules:
                    ...
                    for rule in bucket.lifecycle_rules:
                        ...
                        if "created_before" in condition:
                            rule_dict["condition"]["created_before"] = str(
                                condition["created_before"]
                            )
                        if "matches_storage_class" in condition:
                            rule_dict["condition"]["matches_storage_class"] = condition[
                                "matches_storage_class"
                            ]
                        if "num_newer_versions" in condition:
                            rule_dict["condition"]["num_newer_versions"] = condition[
                                "num_newer_versions"
                            ]
                ...
                if self.config.get("include_iam_policies", False):
                    try:
                        ...
                        logger.warning(
                            f"Could not fetch IAM policy for bucket {bucket.name}: {str(e)}"
                        )
    ...
    def transform(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "public_access_prevention" in raw_data:
                config["public_access_prevention"] = raw_data[
