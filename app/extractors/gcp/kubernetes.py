...
class GCPKubernetesExtractor(BaseExtractor):
        ...
        with ThreadPoolExecutor(
            ...
            tasks = [
                loop.run_in_executor(
                    executor, self._extract_zone_clusters, zone, gcp_session
                )
                for zone in zones
            ]
    ...
    def _extract_zone_clusters(
        ...
        try:
            ...
            for cluster in clusters:
                ...
                if cluster_name:
                    node_pools = self._extract_node_pools(
                        zone, cluster_name, gcp_session
                    )
                    ...
                    "addons_config": (
                        {
                            "http_load_balancing": {
                                "disabled": (
                                    ...
                                    and cluster.addons_config.http_load_balancing
                                    else False
                                ),
                            },
                            "horizontal_pod_autoscaling": {
                                "disabled": (
                                    cluster.addons_config.horizontal_pod_autoscaling.disabled
                                    if cluster.addons_config
                                    and cluster.addons_config.horizontal_pod_autoscaling
                                    else False
                                ),
                            },
                            "kubernetes_dashboard": {
                                "disabled": (
                                    ...
                                    if cluster.addons_config
                                    and cluster.addons_config.kubernetes_dashboard
                                    else False
                                ),
                            },
                            "network_policy_config": {
                                "disabled": (
                                    cluster.addons_config.network_policy_config.disabled
                                    if cluster.addons_config
                                    and cluster.addons_config.network_policy_config
                                    else False
                                ),
                            },
                        }
                    ...
                    "private_cluster_config": (
                        {
                            "enable_private_nodes": (
                                cluster.private_cluster_config.enable_private_nodes
                            ...
                            "master_ipv4_cidr_block": (
                                cluster.private_cluster_config.master_ipv4_cidr_block
                                if cluster.private_cluster_config
                                else ""
                            ),
                        }
                    ...
                    "maintenance_policy": (
                        {
                            "window": {
                                "start_time": (
                                    cluster.maintenance_policy.window.start_time
                                    if cluster.maintenance_policy
                                    and cluster.maintenance_policy.window
                                    else ""
                                ),
                                "end_time": (
                                    cluster.maintenance_policy.window.end_time
                                    if cluster.maintenance_policy
                                    and cluster.maintenance_policy.window
                                    else ""
                                ),
                    ...
                    "autoscaling": (
                        {
                            "enable_node_autoprovisioning": (
                                cluster.autoscaling.enable_node_autoprovisioning
                            ...
                            "resource_limits": (
                                [
                                    ...
                                    for limit in cluster.autoscaling.resource_limits
                                ]
                                if cluster.autoscaling
                                else []
                            ),
    ...
    def _extract_node_pools(
        self, zone: str, cluster_name: str, gcp_session: GCPSession
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for node_pool in response.node_pools:
                node_pool_dict = {
                    ...
                    "config": (
                        {
                            ...
                            "disk_size_gb": (
                                node_pool.config.disk_size_gb if node_pool.config else 0
                            ),
                            "oauth_scopes": (
                                [scope for scope in node_pool.config.oauth_scopes]
                                if node_pool.config
                                else []
                            ),
                            "service_account": (
                                node_pool.config.service_account
                                if node_pool.config
                                else ""
                            ),
                            "metadata": (
                                dict(node_pool.config.metadata)
                                if node_pool.config and node_pool.config.metadata
                                else {}
                            ),
                            "image_type": (
                                node_pool.config.image_type if node_pool.config else ""
                            ),
                            "labels": (
                                dict(node_pool.config.labels)
                                if node_pool.config and node_pool.config.labels
                                else {}
                            ),
                            "tags": (
                                [tag for tag in node_pool.config.tags]
                                if node_pool.config and node_pool.config.tags
                                else []
                            ),
                            "preemptible": (
                                node_pool.config.preemptible
                                if node_pool.config
                                else False
                            ),
                            "accelerators": (
                                [
                                    ...
                                    for acc in node_pool.config.accelerators
                                ]
                                if node_pool.config and node_pool.config.accelerators
                                else []
                            ),
                        }
                        if node_pool.config
                    ...
                    "instance_group_urls": [
                        url for url in node_pool.instance_group_urls
                    ],
                    "autoscaling": (
                        {
                            "enabled": (
                    ...
                    "management": (
                        {
                            "auto_upgrade": (
                                node_pool.management.auto_upgrade
                                if node_pool.management
                                else False
                            ),
                            "auto_repair": (
                                node_pool.management.auto_repair
                                if node_pool.management
                                else False
                            ),
                        }
            ...
            logger.error(
                f"Error extracting node pools for cluster {cluster_name} in zone {zone}: {e}"
            )
        ...
        try:
            ...
            if "cluster" in resource_type_suffix:
                config = {
                    ...
                    "initial_cluster_version": raw_data.get(
                        "initial_cluster_version", ""
                    ),
                    "current_master_version": raw_data.get(
                    ...
                    "private_cluster_config": raw_data.get(
                        "private_cluster_config", {}
                    ),
