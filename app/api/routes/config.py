...
async def get_all_config():
    ...
    try:
        ...
        if not settings.database_enabled:
            raise HTTPException(status_code=400, detail="Database configuration is not enabled")
...
async def update_config_value(key: str, value: Dict[str, Any], description: Optional[str] = None):
...
async def delete_config_value(key: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to delete configuration value")
...
async def reload_config():
    ...
    try:
        ...
        # Get fresh settings
        settings = get_settings()
        return {"message": "Configuration reloaded successfully", "environment": settings.environment}
    except Exception as e:
