...
from app.core.exceptions import ExtractionError, TransportError
from app.transport.base import TransportWithSend
...
class ExtractionOrchestrator:
    ...
    async def run_extraction(self, 
        ...
        asyncio.create_task(self._execute_job(job, services, regions, filters, batch_size))
    ...
    async def _extract_service(self, 
                               extractor,
                               regions: Optional[List[str]],
    ...
    async def _send_artifacts(self, job: Job, artifacts: List[Dict[str, Any]], batch_size: int):
        ...
        for i in range(0, len(artifacts), batch_size):
            ...
            logger.info(f"Sending batch {batch_num}/{total_batches} ({len(batch)} artifacts)")
            ...
            for idx, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    job.errors.append(f"Artifact {batch[idx].get('resource_id')}: {str(result)}")
                else:
