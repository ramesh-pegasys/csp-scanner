    ...
    def __init__(
        ...
        if credentials:
            ...
            try:
                ...
                logger.error("google-auth not installed. Please install: pip install google-auth")
                raise
            except Exception as e:
                logger.error(f"Failed to load GCP credentials from {credentials_path}: {e}")
                raise
        else:
    ...
    def _create_client(self, service: str, region: Optional[str] = None) -> Any:
        ...
        try:
            if service == "compute":
                ...
                return functions_v1.CloudFunctionsServiceClient(credentials=self.credentials)
